...
Materials = {
    ...
    "fused_silica": lambda x: tf.sqrt(
        1
        + 0.6961663 * x ** 2 / (x ** 2 - 0.004679148)
        + 0.4079426 * x ** 2 / (x ** 2 - 0.013512063)
        + 0.8974794 * x ** 2 / (x ** 2 - 97.934002538)
    ),
    ...
    "flint": lambda x: tf.sqrt(
        1
        + 1.34533359 * x ** 2 / (x ** 2 - 0.00997743871)
        + 0.209073176 * x ** 2 / (x ** 2 - 0.0470450767)
        + 0.937357162 * x ** 2 / (x ** 2 - 111.886764)
    ),
    "crown": lambda x: tf.sqrt(
        1
        + 1.1273555 * x ** 2 / (x ** 2 - 0.00720341707)
        + 0.124412303 * x ** 2 / (x ** 2 - 0.0269835916)
...
def triangleMatrix(size, subsize=None):
    ...
    if size % subsize != 0:
        raise ValueError(
            "triangleMatrix: size {} not divisible by subsize {}".format(size, subsize)
        )
    return np.array(
        [
            [1 if x <= y and x // subsize == y // subsize else 0 for x in range(size)]
            for y in range(size)
        ],
        dtype=np.float64,
    )
...
def LensSurface(
    ...
    with tf.name_scope("buildLensSurfaces") as scope:
        ...
        return tf.stack(
            [xpos, ypos, angleStart, angleEnd, radius, materialIn, materialOut], axis=1
        )
...
def LensWithThicknessConstraint(
    position,
    power,
    aperatureRadius,
    facingLeft,
    ...
    if power.shape[0] < 2:
        raise ValueError(
            "LensWithThicknessConstraint: length of power must be at least 2."
        )
    ...
    if spacing.shape[0] != power.shape[0] - 1:
        raise ValueError(
            "LensWithThicknessConstraint: length of spacing must be one less than the other "
            "parameters."
        )
    ...
    if mode == "left":
        ...
        raise ValueError(
            "LensWithThicknessConstraint: mode {} invalid.  Must be 'left' or 'center'.".format(
                mode
            )
        )
        ...
        if abs(omega) >= 2 * criticalAngle:
            ...
            return (
                ...
                + math.asin(
                    nr
                    * math.sin(omega)
                    / math.sqrt(nr * nr - 2 * nr * math.cos(omega) + 1)
                )
            ) % (2 * PI)
    else:
...
def RandomLambertianSource(
    ...
    try:
        wavelengthIndices = tf.random.uniform(
            (count,), minval=0, maxval=len(allowedWavelengths), dtype=tf.int64
        )
    ...
    startParameter = tf.random_uniform(
        (count,), minval=-1.0, maxval=1.0, dtype=tf.float64
    )
...
def segmentParameterSmoother(parameter, strength=1.0, filterSize=5, sigma=1.5):
    ...
    if filterSize % 2 == 0:
        raise ValueError(
            "segmentParameterSmoother: filter size {} must be odd.".format(filterSize)
        )
    ...
    return tf.assign(
        parameter, smoothedParameter * strength + parameter * (1 - strength)
    )
