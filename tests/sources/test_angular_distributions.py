...
@pytest.mark.parametrize("min_angle,max_angle", [
    (0, PI/4.0),
    (-PI/2.0, PI/2.0),
...
def test_with_constants(session, distribution_type, min_angle, max_angle, sample_count, name="test_distribution"):
    distribution = distribution_type(min_angle, max_angle, sample_count, name)
    distribution.build()
    session.run(distribution.angles)
...
def test_with_feeds(session, distribution_type, min_angle, max_angle, sample_count, name="test_distribution"):
    distribution = distribution_type(feed_min_angle, feed_max_angle, feed_sample_count, name)
    distribution.build()
    session.run(distribution.angles, feed_dict={
