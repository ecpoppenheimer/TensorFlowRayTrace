...
"""
def rayTraceSinglePass(rays, boundarySegments, boundaryArcs, targetSegments, targetArcs, materials, epsilion=1e-6):
    ...
    with tf.name_scope("rayTraceSingle") as scope:
        ...
        with tf.name_scope("rayRange") as scope:
            rayRange = tf.range(tf.shape(rays, out_type=tf.int64)[0], dtype=tf.int64, name="rayRange")
        ...
        with tf.name_scope("segmentTargetJoining") as scope:
            if b_usingSegments:
                opticalSegmentCount = tf.cast(tf.shape(boundarySegments)[0], dtype=tf.int64)
            else:
            ...
            if targetSegments is not None:
                ...
                if b_usingSegments:
                    boundarySegments = tf.concat((boundarySegments, targetSegments), 0, 
        ...
        with tf.name_scope("arcTargetJoining") as scope:
            ...
            if targetArcs is not None:
                ...
                if b_usingArcs:
                    boundaryArcs = tf.concat((boundaryArcs, targetArcs), 0, name="joinedBoundaryArcs")
        ...
        if b_usingSegments:
            with tf.name_scope("ray-SegmentIntersection") as scope:
                ...
                with tf.name_scope("raw_v_parameter") as scope:
                    ...
                    validSegmentIntersection = tf.greater_equal(tf.abs(denominator), epsilion)
                    safe_value = tf.ones_like(denominator)
                    safe_denominator = tf.where(validSegmentIntersection, denominator, safe_value)
                    
                    segmentV = tf.where(validSegmentIntersection, (ya*(xb1-xa1)-xa*(yb1-ya1))/safe_denominator, 
                        safe_value)
                with tf.name_scope("raw_u_parameter") as scope:
                    segmentU = tf.where(validSegmentIntersection, (xb*(ya1-yb1)-yb*(xa1-xb1))/safe_denominator, 
                        safe_value)
                ...
                with tf.name_scope("selectClosestValidIntersection") as scope:
                    validSegmentIntersection = tf.logical_and(validSegmentIntersection,
                        tf.greater_equal(segmentV, -epsilion))
                    validSegmentIntersection = tf.logical_and(validSegmentIntersection,
                        tf.less_equal(segmentV, 1.0+epsilion))
                    validSegmentIntersection = tf.logical_and(validSegmentIntersection,
                        tf.greater_equal(segmentU, epsilion))
                ...
                with tf.name_scope("segmentU") as scope:
                    ...
                    raySegmentIndices = tf.argmin(segmentU, axis=0, name="raySegmentIndices")
                    
                    # intersectIndicesSquare is a set of indices that can be used with gather_nd to select 
                    # positions out of the grid tensors
                    intersectIndicesSquare = tf.transpose(tf.stack([raySegmentIndices, rayRange]))
                    
                    # the u parameter for ray intersections, after filtering and processing
                    segmentU = tf.gather_nd(segmentU, intersectIndicesSquare, name="segmentU")
                    
                    # package and pair the boundary segments with the rays that intersect with them
                    boundarySegments = tf.gather(boundarySegments, raySegmentIndices, 
                        name="boundarySegments")
            ...
            with tf.name_scope("ray-ArcIntersection") as scope:
                ...
                with tf.name_scope("raw_u_parameter") as scope:
                    ...
                    with tf.name_scope("cullNegativeU") as scope:
                        uminus_valid = tf.logical_and(uminus_valid, tf.greater_equal(uminus, epsilion))
                        uplus_valid = tf.logical_and(uplus_valid, tf.greater_equal(uplus, epsilion))
                ...
                with tf.name_scope("selectValid_v") as scope:
                    ...
                    vminus_valid = tf.less_equal(tf.floormod(vminus-thetaStart, 2*PI), 
                        tf.floormod(thetaEnd - thetaStart, 2*PI))
                    uminus_valid = tf.logical_and(vminus_valid, uminus_valid)
                    
                    vplus_valid = tf.less_equal(tf.floormod(vplus-thetaStart, 2*PI), 
                        tf.floormod(thetaEnd - thetaStart, 2*PI))
                ...
                with tf.name_scope("choosePlusOrMinus") as scope:
                    ...
                    inf = 2*safe_value*tf.reduce_max([tf.reduce_max(uminus), tf.reduce_max(uplus)])
                    ...
                    uplus_valid = tf.logical_and(uplus_valid, tf.logical_not(choose_uminus))
                ...
                with tf.name_scope("arcU_and_arcV") as scope:
                    ...
                    # intersectIndicesSquare is a set of indices that can be used with gather_nd to select 
                    # positions out of the grid tensors
                    intersectIndicesSquare = tf.transpose(tf.stack([rayArcIndices, rayRange]))
                    ...
                    boundaryArcs = tf.gather(boundaryArcs, rayArcIndices, 
                        name="boundaryArcs")          
                
        # determine which rays are dead
        with tf.name_scope("deadRays") as scope:
            if b_usingSegments and b_usingArcs:
                deadRays = tf.boolean_mask(rays, tf.logical_not(tf.logical_or(rayArcMask, raySegmentMask)), 
                    name="deadRays")
            else:
                if b_usingSegments:
                    deadRays = tf.boolean_mask(rays, tf.logical_not(raySegmentMask), 
                        name="deadRays")
                elif b_usingArcs:
                    deadRays = tf.boolean_mask(rays, tf.logical_not(rayArcMask), 
                        name="deadRays")
                else:
                    raise RuntimeError("rayTraceSinglePass: no boundaries provided for raytracing")
                
        # select between segment and arc intersections
        with tf.name_scope("arc_segment_selection") as scope:
            if b_usingSegments and b_usingArcs:
                chooseSegment = tf.logical_and(tf.less(segmentU, arcU), raySegmentMask, name="chooseSegment")
                chooseSegment = tf.logical_or(chooseSegment, 
                    tf.logical_and(raySegmentMask, tf.logical_not(rayArcMask)))
                
                chooseArc = tf.logical_and(tf.logical_not(chooseSegment), rayArcMask, name="chooseArc")
                chooseArc = tf.logical_or(chooseArc,
        ...
        with tf.name_scope("rayProjection") as scope: 
            if b_usingSegments:
                with tf.name_scope("segments") as scope: 
                    ...
                    reactedRays_Segment = tf.stack([xstart, ystart, xend, yend, rays[:,4], rays[:,5]], 
                        axis=1)
            ...
            if b_usingArcs:    
                with tf.name_scope("arcs") as scope: 
                    ...
                    reactedRays_Arc = tf.stack([xstart, ystart, xend, yend, rays[:,4], rays[:,5]], axis=1)
        ...
        with tf.name_scope("finishedRays") as scope:
            ...
            if b_usingSegments:
                finishedSegmentMask = tf.greater_equal(raySegmentIndices, opticalSegmentCount, 
                    name="finishedSegmentMask")
            ...
            if b_usingArcs:
                finishedArcMask = tf.greater_equal(rayArcIndices, opticalArcCount, name="finishedArcMask")
        ...
        with tf.name_scope("reactedRays") as scope:
            ...
            if b_usingSegments:
                chooseSegment = tf.logical_and(tf.logical_not(finishedSegmentMask), chooseSegment)
                reactedRays_Segment = tf.boolean_mask(reactedRays_Segment, chooseSegment, 
                    name="reactedRays_Segment")
                boundarySegments = tf.boolean_mask(boundarySegments, chooseSegment, name="boundarySegments")
                reactedRays = tf.cond(
            ...
            if b_usingArcs:
                ...
                reactedRays_Arc = tf.boolean_mask(reactedRays_Arc, chooseArc, name="reactedRays_Arc")
                arcV = tf.boolean_mask(arcV, chooseArc, name="arcV")
                boundaryArcs = tf.boolean_mask(boundaryArcs, chooseArc, name="boundaryArcs")
                reactedRays = tf.cond(
        ...
        with tf.name_scope("norm") as scope:
            ...
            if b_usingSegments:
                normSegment = tf.atan2(boundarySegments[:,3]-boundarySegments[:,1],
                    boundarySegments[:,2]-boundarySegments[:,0], name="normSegment") + PI/2
                norm = tf.cond(
                    tf.reduce_any(chooseSegment), 
            ...
            if b_usingArcs:
                normArc = tf.where(tf.less(boundaryArcs[:,4], 0), arcV + PI, arcV, name="normArc")
        ...
        with tf.name_scope("refractiveIndex") as scope:
            ...
            nstack = tf.stack([each(wavelengths) for each in materials], axis=1, name="nstack")
            rayRange = tf.range(tf.shape(reactedRays)[0], dtype=tf.int32, name="rayRange")
            
            # select just the correct entry for n_in and n_out
            if b_usingSegments and b_usingArcs:
                n_in_indices = tf.concat([boundarySegments[:,4], boundaryArcs[:,5]], axis=0, name="n_in_indices")
            else:
                if b_usingSegments:
            ...
            if b_usingSegments and b_usingArcs:
                n_out_indices = tf.concat([boundarySegments[:,5], boundaryArcs[:,6]], axis=0, name="n_out_indices")
            else:
                if b_usingSegments:
...
def react(reactedRays, norm, n_in, n_out):
    with tf.name_scope("ray_reactions") as scope: 
        ...
        with tf.name_scope("theta1") as scope:
            theta1 = tf.atan2(reactedRays[:,1]-reactedRays[:,3],
                    reactedRays[:,0]-reactedRays[:,2], name="theta1")
            theta1 =  norm - theta1
        ...
        internalMask = tf.greater_equal(tf.abs(theta1), PI/2, name="internalRefractionsMask")
        ...
        theta1 = tf.where(internalMask, theta1+PI, theta1, "theta1Internal_ExternalSelector")

        theta2 = tf.multiply(tf.sin(theta1), n, name="theta2")
        
        # the where selects between TIR and internal refraction
        theta2 = tf.where(tf.less_equal(tf.abs(theta2), 1.0), norm - tf.asin(theta2), norm + theta1 + PI, 
            name="TIR_selector")
        ...
        return tf.stack([xstart, ystart, xend, yend, reactedRays[:,4], reactedRays[:,5]], axis=1, 
            name="activeRays")
...
def rayTrace(rays, boundarySegments, boundaryArcs, targetSegments, targetArcs, materials, maximumLoopDepth, 
    b_terminateIfFinished=True, epsilion=1e-6):
    
    # error check the shape of the inputs
    if rays.shape[1] != 5:
        raise ValueError("rayTrace: rays had incorrect shape {}.  Shape must be (..., 5)"
            .format(rays.shape))
    if boundarySegments is not None and boundarySegments.shape[1] != 6:
        raise ValueError("rayTrace: boundarySegments had incorrect shape {}.  Shape must be (..., 6)"
            .format(boundarySegments.shape))
    if boundaryArcs is not None and boundaryArcs.shape[1] != 7:
        raise ValueError("rayTrace: boundaryArcs had incorrect shape {}.  Shape must be (..., 7)"
            .format(boundarySegments.shape))
    if targetSegments is not None and targetSegments.shape[1] != 4:
        raise ValueError("rayTrace: targetSegments had incorrect shape {}.  Shape must be (..., 4)"
            .format(targetSegments.shape))
    if targetArcs is not None and targetArcs.shape[1] != 5:
        raise ValueError("rayTrace: targetArcs had incorrect shape {}.  Shape must be (..., 5)"
            .format(targetArcs.shape))
    ...
    def stoppingCondition(counter, reactedRays, activeRays, finishedRays, deadRays):
        with tf.name_scope("loopCondition") as scope:
            if b_terminateIfFinished:
                ...
                return tf.logical_and(tf.less(counter, maximumLoopDepth), tf.not_equal(activeRayCount, 0))
            else:
    ...
    def loopBody(counter, reactedRays, activeRays, finishedRays, deadRays):
        with tf.name_scope("incrementCounter") as scope:
            counter += 1
        reactedRaysOut, activeRays, finishedRaysOut, deadRaysOut = rayTraceSinglePass(activeRays, 
            boundarySegments, boundaryArcs, targetSegments, targetArcs, materials, epsilion=epsilion)
            
        reactedRays = tf.concat([reactedRays, reactedRaysOut], 0, name="concatReactedRays")
        finishedRays = tf.concat([finishedRays, finishedRaysOut], 0, name="concatFinishedRays")
