...
def line_intersect(first_lines, second_lines, name=None, epsilion=1e-10):
    ...
    with tf.name_scope(name, default_name="line_intersect"):
        first_lines = tf.ensure_shape(
            first_lines, (None, 4), name="Check_First_Lines_Shape"
        )
        second_lines = tf.ensure_shape(
            second_lines, (None, 4), name="Check_Second_Lines_Shape"
        )
        ...
        return raw_line_intersect(
            (x1s, y1s, x1e, y1e), (x2s, y2s, x2e, y2e), epsilion=epsilion
        )
...
def raw_line_intersect(
    first_coordinates, second_coordinates, epsilion=1e-10, name=None
):
    """
...
def line_circle_intersect(lines, circles, name=None, epsilion=1e-10):
    ...
    with tf.name_scope(name, default_name="line_circle_intersect_1_to_1"):
        ...
        return raw_line_circle_intersect(
            (xs, ys, xe, ye), (xc, yc, r), epsilion=epsilion
        )
...
def line_circle_intersect_1to1(lines, circles, name=None, epsilion=1e-10):
    ...
    with tf.name_scope(name, default_name="line_circle_intersect_1to1"):
        ...
        circles = tf.ensure_shape(circles, (None, 3), name="Check_Circles_Shape")
        return raw_line_circle_intersect(
            tf.unstack(lines, axis=-1), tf.unstack(circles, axis=-1), epsilion=epsilion
        )
...
def raw_line_circle_intersect(lines, circles, epsilion=1e-10, name=None):
    """
    ...
    name : str, optional
        ...
        function.  Defaults to "line_circle_intersect".
    ...
    with tf.name_scope(name, default_name="line_circle_intersect"):
        ...
        return (
            {"coords": (xplus, yplus), "valid": uplus_valid, "u": uplus, "v": vplus},
            {
...
def ray_reaction(rays, norm, n_in, n_out, new_ray_length=1.0, name="ray_reaction"):
    """
    ...
    other parameters.  It can have any shape before the last dimension, which can 
    index individual rays and may include batches.  Norm, n_in, and n_out must have 
    the same shape as rays does except for rays' last dimension.  In other words, 
    rays is unstacked along its last dimension, and norm, n_in, and n_out must be 
    broadcastable with each of the unstacked tensors.
    ...
    name : str, optional
        The name of the name scope to use for the operations generated by this
        function.  Defaults to "ray_reaction".
    ...
    """
    with tf.name_scope(name, default_name="ray_reaction"):
        ...
        theta1 = tf.where(
            internal_mask, theta1 + PI, theta1, "theta1_internal_external_selector"
        )
...
def angle_in_interval(angle, start, end, name=None, epsilion=1e-10):
    ...
    [0, 2PI].  If you set the optional parameter mod_reduce to false, this function
    will throw a modulo 2PI operation in front of each of its inputs, to guarentee this
    condition.  However, this operation is slow and may be unnecessary, so it might be
    better to ensure that your angles fall within [0, 2PI].
    
    Parameters
    ----------
    angle : float tensor-like
        The angle to check.
    start : float tensor-like
        The start of the interval.  Must be broadcastable to the same shape as angle.
    end : float tensor-like
        The end of the interval.  Must be broadcastable to the same shape as angle.
    name : str, optional
        The name of the name scope to use for the operations generated by this
        function.  Defaults to "angle_in_interval".
    epsilion : float, optional
        A small number, used only to check if the interval includes all angles 
        (like 0 -> 2PI).  If the difference between start and end is less than this
        value, the interval is considered to include all angles.  Necessary to avoid
        rounding errors when taking angle % 2PI.
    ...
    with tf.name_scope(name, default_name="angle_in_interval"):
        angle = tf.mod(angle - start, 2 * PI)
        end = tf.mod(end - start, 2 * PI)
        angle_in_interval = tf.less_equal(angle, end)

        # check if the interval is a full circle
        end = tf.where(tf.less_equal(end, PI), end, end - 2 * PI)
        full_circle = tf.less(tf.abs(end), epsilion)

        return tf.logical_or(angle_in_interval, full_circle)
