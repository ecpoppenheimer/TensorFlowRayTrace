...
class CanyonOptimizer(object):
    def __init__(self, baseStepSize=1.0, momentum=.95, stepSizeGrowthFactor=1.1, stepSizeShrinkFactor=.5):
    ...
    def getUpdateStep(self, grads_and_vars, quality):
        ...
        for gradient, variable in grads_and_vars:
            ...
            parameters["previousStep"] = np.zeros(shape=gradient.shape, dtype=np.float64)
            ...
            variablePlaceholder = tf.placeholder(tf.float64, shape=variable.shape, 
            ...
            def stepFunction(session, fd, p=parameters):
                ...
                # save the old values
                p["previousValue"], p["previousQuality"] = session.run([variable, quality], feed_dict=fd)
                ...
                f = fd
                f[variablePlaceholder] = p["previousValue"] + self.momentum * p["previousStep"]
                ...
                if newQuality < p["previousQuality"]:
                    ...
                    print("step in wrong direction.  StepSizeFactor: ", p["stepSizeFactor"])
                    session.run(assign, feed_dict={variablePlaceholder: p["previousValue"]})
