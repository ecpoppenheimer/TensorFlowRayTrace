...
class PointSource(SourceBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(standard_domains={"angle"}, **kwargs)
    ...
    def center(self, val):
        ...
        self._center = tf.cast(val, tf.float64)
    ...
    def central_angle(self, val):
        ...
        self._central_angle = tf.cast(val, tf.float64)
...
class AngularSource(SourceBase):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(standard_domains={"base_point", "angle"}, **kwargs)
...
class ChainedSource(SourceBase):
    """
    A set of rays that span two sets of endpoints.
    
    This source is distinct from AperatureSource in that it uses points that were generated
    from chained distributions - that is, distributions that are correlated, or which were
    generated from the same base uniform random sample.  While Aperature source is ideal for 
    diffuse sources where the start and end points of the rays are uncorrelated,like a
    LED, this source is more suited for a laser.
    
    Also unlike the other sources, this source does not operate on point clouds, but rather
    on the distributions themselves.
