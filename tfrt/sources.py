...
class PointSource(Source2D):
    ...
    The source will always generate ranks in its signature, in this case, "angle_ranks".  
    It will do this even if its consumed distribution does not define ranks, in which case 
    the fields will contain zeros.  This field differs from the ranks described by the 
    distribution if the source is dense, since the angles will be combined with the 
    wavelengths.
    ...
    def _update(self):
        ...
        if self.dense:
            self._angles, self._processed_wavelengths, self["angle_ranks"] = \
                self._make_dense(
...
class AngularSource(Source2D):
    ...
    The source will always generate ranks in its signature, in this case, "angle_ranks"
    and "point_ranks".  It will do this even if its consumed distributions do not define
    ranks, in which case these fields will contain zeros.  These fields differ from the 
    ranks described by the distributions if the source is dense, since the points 
    will be combined with the wavelengths and angles.
    ...
    def _update(self):
        ...
        if self.dense:
            ...
            if self._base_point_distribution.ranks is not None:
                point_ranks = self._base_point_distribution.ranks
            else:
                point_ranks = tf.zeros_like(
                    self._base_point_distribution.base_points_x
                )
                
            self._angles, self._base_points_x, self._base_points_y, \
                self._processed_wavelengths, self["angle_ranks"], \
                self["point_ranks"] = \
                self._make_dense(
                    self._angular_distribution.angles,
                    self._base_point_distribution.base_points_x,
...
class AperatureSource(Source2D):
    ...
    The source will always generate ranks in its signature, in this case, "start_ranks"
    and "end_ranks".  It will do this even if its consumed distributions do not define
    ranks, in which case these fields will contain zeros.  These fields differ from the 
    ranks described by the distributions if the source is dense, since the start points 
    will be combined with the wavelengths and end points.
    ...
    def _update(self):
        ...
        if self.dense:
            self["x_start"], self["y_start"], self["x_end"], self["y_end"], \
                self["wavelength"], self["start_ranks"], \
                self["end_ranks"] = self._make_dense(
                    self._start_point_distribution.base_points_x,
            ...
            self["x_start"], self["y_start"], self["x_end"], self["y_end"], \
                self["wavelength"], self["start_ranks"], \
                self["end_ranks"] = self._make_undense(
                    self._start_point_distribution.base_points_x,
                    self._start_point_distribution.base_points_y,
                    self._end_point_distribution.base_points_x,
                    self._end_point_distribution.base_points_y,
